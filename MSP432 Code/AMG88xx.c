...
void AMG88xxInit(uint8_t addr)
{
    ...
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P3, GPIO_PIN6);          // Activate the pull up resistors
    ...
    I2C_setSlaveAddress(EUSCI_B2_BASE, 0x69);
    I2C_setMode(EUSCI_B2_BASE, EUSCI_B_I2C_TRANSMIT_MODE);      // Master in transmit mode
...
//        I2C_clearInterruptFlag(EUSCI_B2_BASE,
//            EUSCI_B_I2C_TRANSMIT_INTERRUPT0);
//
//    while (I2C_isBusBusy(EUSCI_B2_BASE));

    // Enter Normal Mode
    I2C_masterSendMultiByteStart(EUSCI_B2_BASE, Reg_PowerControl);
    I2C_masterSendMultiByteFinish(EUSCI_B2_BASE, PowerControl_NormalMode);
    // Software Reset
    I2C_masterSendMultiByteStart(EUSCI_B2_BASE, Reg_Reset);
    I2C_masterSendMultiByteFinish(EUSCI_B2_BASE, Reset_InitialReset);
    // Disable interrupts
    I2C_masterSendMultiByteStart(EUSCI_B2_BASE, Reg_InterruptState);
    I2C_masterSendMultiByteFinish(EUSCI_B2_BASE, InterruptIntModeDiff | InterruptIntEnableReAct);
    // Set to 10 Frames a second
    I2C_masterSendMultiByteStart(EUSCI_B2_BASE, Reg_FrameRate);
    I2C_masterSendMultiByteFinish(EUSCI_B2_BASE, FrameRate10FPS);
}
